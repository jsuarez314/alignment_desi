...
@numba_jit
def _recursive_build(i_node, idx_start, idx_end,data, node_centroids, node_radius, idx_array,node_idx_start, node_idx_end, node_is_leaf,n_nodes, leaf_size):
    ...
    if i_child >= n_nodes:
        ...
        if idx_end - idx_start > 2 * leaf_size:
            ...
            # warnings.warn("Internal: memory layout is flawed: not enough nodes allocated")
            pass

    elif idx_end - idx_start < 2:
        # again, this shouldn't happen if our memory allocation is correct.
        # warnings.warn("Internal: memory layout is flawed: too many nodes allocated")
        node_is_leaf[i_node] = True
